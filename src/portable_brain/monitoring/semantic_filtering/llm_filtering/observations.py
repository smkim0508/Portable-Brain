...
class ObservationInferencer(ObservationRepository):
    """
    Helper to inference observations from state snapshots (denoised accessibility tree text).
    NOTE: inherits from repository for dependencies.
    """

    async def test_create_new_observation(self, state_snapshots: list[str]) -> Optional[Observation]:
        test_llm_response = await self.llm_client.acreate(
            system_prompt=ObservationPrompts.test_system_prompt,
            user_prompt=ObservationPrompts.get_test_user_prompt(state_snapshots),
    ...
    async def create_new_observation(
        ...
        input: list of state snapshots (denoised formatted text from accessibility tree)
        ...
        new_observation_response: NewObservationLLMResponse = await self.llm_client.acreate(
            ...
            user_prompt=ObservationPrompts.get_create_new_observation_user_prompt(state_snapshots),
    ...
    async def update_observation(self, observation: Observation, state_snapshots: list[str]) -> Optional[Observation]:
        """
        ...
        updated_observation_response = await self.llm_client.acreate(
            ...
            user_prompt=ObservationPrompts.get_update_observation_user_prompt(observation, state_snapshots),
        ...
        logger.info(f"updated observation llm response: {updated_observation_node}, reasoning: {reasoning}")
